...
st.title("Mileage Predictor of Vehicle")
st.markdown("##### Want to check mileage of you chosen car?\n##### So let's try evaluating the mileage ")
st.image("mileage.jpg")
st.markdown("#### Fill in the details below and get your predicted mileage of the car #####")


a = st.number_input("Enter the number of Cylinders in cc of vehicle",20,1000)
b = st.number_input("Enter the Displacement in cc of vehicle",500,999999)
c = st.number_input("Enter the Horsepower in ps of vehicle",400,600000)
d = st.number_input("Enter the Weight in Kg of vehicle",200,7890000)
if st.button('Predict'):
    out = mil([[a, b, c, d, 70, 20]])

    st.write(out[0], 'is the predicted mileage in kmpl')
